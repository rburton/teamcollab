...
public class ConversationController {
    ...
    public String show(@PathVariable Long id, @NonNull @AuthenticationPrincipal User user, @NonNull Model model) {
        ...
        // Add information about which messages are bookmarked
        model.addAttribute("bookmarkedMessageIds", 
            conversation.getMessages().stream()
                .filter(message -> bookmarkService.isMessageBookmarked(user.getId(), message.getId()))
                .map(Message::getId)
                .toList());
